...
depth_dataset_path = sprintf('../eval_sim_data/old_nn/depth/%sdb/',snr_db);
...
for ii = 1:numel(depths_subdirs)
    ...
    for jj = 1:numel(C)
        ...
        elseif strcmp(C{jj},'sources.npy')
            sources = double(readNPY(fullfile(depth_dataset_path,depths_subdirs{ii},C{jj})));
    ...
    for kk=1:n_samples
        ...
        [~,location] =  create_source_activation_vector(sources(:,kk),'g_t',cd_matrix);    
        
        % prediction of th cnn
        cnn_pred = create_source_activation_vector(cnn_predictions(:,kk),'nn',cd_matrix);        
 ...
 save(sprintf('./eval_results/old_nn/depth_eval/%sdb/mle_cnn.mat',snr_db),'mle_cnn');
 save(sprintf('./eval_results/old_nn/depth_eval/%sdb/mle_s_loreta.mat',snr_db),'mle_s_loreta');
 save(sprintf('./eval_results/old_nn/depth_eval/%sdb/mle_dipole_scan.mat',snr_db),'mle_dipole_scan');
 save('./eval_results/old_nn/depth_eval/depth.mat','depth');
...
% clear; close all; clc;
% snr_db = '-5';
load(sprintf('./eval_results/old_nn/depth_eval/%sdb/mle_cnn.mat',snr_db));
load(sprintf('./eval_results/old_nn/depth_eval/%sdb/mle_s_loreta.mat',snr_db));
load(sprintf('./eval_results/old_nn/depth_eval/%sdb/mle_dipole_scan.mat',snr_db));
load('./eval_results/old_nn/depth_eval/depth.mat');
